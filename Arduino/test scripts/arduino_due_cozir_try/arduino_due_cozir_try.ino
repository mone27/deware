/*
AN128_ardunio_cozir CO2 Demonstration code 11/22/2016 Runs on Ardunio UNO, MEGA or MEGA2560
...
void setup() {
  ...
  Serial3.begin(9600); // Start serial communications with sensor
  Serial.println("my seraial started");
  Serial3.println("K 0");  // Set Command mode
  Serial3.println("M 6"); // send Mode for Z and z outputs
  // "Z xxxxx z xxxxx" (CO2 filtered and unfiltered)

  Serial3.println("K 1");  // set streaming mode
...
void loop() {
 ...
 format_output();
  ...
  indexb = 8;  // In ASCII buffer, filtered value is offset from raw by 8 bytes
  format_output();
...
while(buffer[ind-1] != 0x0A){  // Read sensor and fill buffer up to 0XA = CR
 ...
 int format_output(void){ // read buffer, extract 6 ASCII chars, convert to PPM and print
   co2 = buffer[15-indexb]-0x30;
  co2 = co2+((buffer[14-indexb]-0x30)*10);
  co2 +=(buffer[13-indexb]-0x30)*100;
  co2 +=(buffer[12-indexb]-0x30)*1000;
  co2 +=(buffer[11-indexb]-0x30)*10000;
  Serial.print("\n CO2 = ");
